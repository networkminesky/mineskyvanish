...
public class ExceptionLogger {
    ...
    public static void logException(Throwable e, MineSkyVanish plugin) {
        try {
            ...
            if (plugin.getConfigMgr().isSettingsUpdateRequired()
                ...
                if (realEx) plugin.log(loggingLevel, "You have an outdated configuration, " +
                        "regenerating it by using '/msv recreatefiles' might fix this problem!");
            ...
            for (Plugin pl : Bukkit.getServer().getPluginManager().getPlugins()) {
                if (pl.getName().equalsIgnoreCase("MineSkyVanish")) continue;
